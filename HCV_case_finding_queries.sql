...
--author: Max Vandromme @ Department of Liver Diseases, Mount Sinai School of Medicine
--last update: September 10, 2020

--Note: these queries were suited for the internal databases of the Mount Sinai Health System.
--There is obviously no guarantee that they should work as such on any other system.
--Notably, the database structure, as well as the lab test names for example, are expected to be different at other sites.
--However, we believe that this could help other teams implement a similar process, should they wish to.
--The prototype pipeline used SQL queries and some Java code for data processing.
--It was later changed to a SQL-only process for convenience and ease of maintenance and distribution.
--Queries use the t-SQL extension syntax.

--The overall flow of this file is:
--    1) Get all HCV RNA labs, antibody labs, and ICD codes
--    2) From these, find "possibly infected" patients
--    3) Get additional data about medications, other labs, etc. to determine most probable infection status
--    4) Report all useful information in one single table as output
